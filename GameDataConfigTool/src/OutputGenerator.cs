...
public class OutputGenerator
{
    ...
    private async Task GenerateDataManagerAsync(GameDataTool.Parsers.GameData data, string outputPath, CodeGeneration config)
    {
        ...
        foreach (var table in data.Tables)
        {
            ...
            managerCode.AppendLine($"                () => {config.Namespace}.{table.Name}Config.InitializeAsync(");
    ...
    private string GenerateConfigDataClass(DataTable table, CodeGeneration config)
    {
        ...
        code.AppendLine($"        /// <summary>");
        code.AppendLine($"        /// Async initialization for WebGL platform");
        code.AppendLine($"        /// </summary>");
        code.AppendLine($"        public static async System.Threading.Tasks.Task InitializeAsync(string binaryPath)");
        code.AppendLine($"        {{");
        code.AppendLine($"            await BaseConfigData<{table.Name}Data>.InitializeAsync(binaryPath);");
        code.AppendLine($"            PostInitialize();");
        code.AppendLine($"        }}");
